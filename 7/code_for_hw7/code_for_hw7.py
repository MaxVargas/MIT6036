...
class Linear(Module):
    ...
    def sgd_step(self, lrate):  # Gradient descent step
        ...
        self.W0 = self.W0 - lrate*np.reshape(np.sum(self.dLdW0,axis=1),(self.n,1))  # Your code
...
class Tanh(Module):  # Layer activation
    ...
    def backward(self, dLdA):  # Uses stored self.A
        return np.multiply(1-self.A**2,dLdA)  # Your code: return dLdZ (?, b)
...
class SoftMax(Module):  # Output activation
    def forward(self, Z):
        self.n,self.d = np.shape(Z)
        self.exps = np.exp(Z)
        self.parts = np.reshape(np.sum(self.exps,axis=0),(1,self.d))
        self.A = self.exps/self.parts
...
class NLL(Module):  # Loss
    ...
    def backward(self):  # Use stored self.Ypred, self.Y
        return (1-self.Y)*self.Ypred-(self.Y)*(1-self.Ypred)  # Your code (?, b)
...
class Sequential:
    ...
    def sgd(self, X, Y, iters=100, lrate=0.005):  # Train
        ...
        for it in range(iters):
            ...
            As = []
            As.append(self.forward(np.reshape(np.array(X[:,i]),(D,1))))
            loss = self.loss.forward(As[0],Y[:,i].reshape(D,1))
