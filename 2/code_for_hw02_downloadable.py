...
def perceptron(data, labels, params={}, hook=None):
    ...
    th = np.zeros((d,1))
    th_0 = np.zeros(1)
    #print(th,data[:,0],data)
    for t in range(0,T):
        for i in range(0,n):
            if labels[0,i]*(np.dot(data[:,i],th)+th_0) <=0:
                th[:,0] = th[:,0] + data[:,i]*labels[0,i]
                th_0 = th_0 + labels[0,i]
    return (th,np.array([[th_0[0]]]))
...
def eval_classifier(learner, data_train, labels_train, data_test, labels_test):
    ...
    #print(labels_test.size,sc/float(labels_test.size))
    return sc/float(labels_test.size)
...
def eval_learning_alg(learner, data_gen, n_train, n_test, it):
    ...
    for i in range(it):
        ...
        #change commenting so that evaluation occurs on the hypothesis data or different non-training data
        #d_test, l_test = data_gen(n_test)
        #sc = sc + eval_classifier(learner, d_train, l_train, d_test, l_test)
        sc = sc + eval_classifier(learner, d_train, l_train, d_train, l_train)
    return sc/it
...
def xval_learning_alg(learner, data, labels, k):
    d = np.array_split(data,k,1)
    l = np.array_split(labels,k,1)
    score = 0
    #print d
    #print np.concatenate(np.delete(d,0,0), 1)
    for j in range(k):
        d_wo_j = np.concatenate(np.delete(d,j,0), 1)
        l_wo_j = np.concatenate(np.delete(l,j,0), 1)
        score = score + eval_classifier(learner, d_wo_j, l_wo_j, d[j], l[j])
    return score/k
...
print eval_learning_alg(averaged_perceptron, gen_flipped_lin_separable(pflip=.1), 20, 20, 500)
