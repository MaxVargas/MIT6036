...
def sgd_step(data, x, lam, step):
    ...
    (lam_u, lam_v) = lam
    # Your code here
    QuadGrad = (r - np.dot(u[a].T,v[i]).item() - b_u[a] - b_v[i])
    QuadGradU, QuadGradV = -QuadGrad*v[i], -QuadGrad*u[a]
    RegUGrad, RegVGrad = lam_u[a]*u[a], lam_v[i]*v[i]
    GradientU, GradientV = QuadGradU + RegUGrad, QuadGradV + RegVGrad
    u[a] = u[a] - step*GradientU
    v[i] = v[i] - step*GradientV
    b_u[a] = b_u[a] - step*(-QuadGrad)
    b_v[i] = b_v[i] - step*(-QuadGrad)
    return x
...
for d in data:
    if d[0] == a:
        user_a_movies.append(d[1])

for i in range(m):
    r = np.dot(u_a.T,v[i]).item() + b_u_a + b_v[i]
    if (i not in user_a_movies) and (i in movies_dict):
        recs.append((i,r))
...
for d in recs[-50:]:
    movie_id = d[0]
    if "Animation" in genres_dict[movie_id]:
        count += 1
print(count)
'''
def compute_sim(m1,m2):
    return np.dot(v[m1].T,v[m2])/(np.linalg.norm(v[m1])*np.linalg.norm(v[m2]))
...
for m in movies_dict:
    angle = compute_sim(m1,m2)
    sim_to_movie.append((m,angle))

sim_to_movie.sort(key=lambda x: x[1])
most_sims = [x[0] for x in sim_to_movie[-11:-1]]
print(most_sims)
print([movies_dict[x] for x in most_sims])
'''

#movies is a list of movie_ids
def avg_sim(movies):
    m = len(movies)
    s,c = 0,0
    for i in range(m):
        for j in range(i+1,m):
            s += compute_sim(movies[i],movies[j])
            c += 1
    return(s/c)


#movie_indices = list(movies_dict.keys())

#print(avg_sim(movie_indices))

def movies_in_genre(genre):
    movies = []
    for m in genres_dict:
        if genre in genres_dict[m]:
            movies.append(m)
    return movies

'''
#Compute the genre with most/least similar movies
max_sim = 2
max_gen = ""
...
print(max_sim,max_gen)
'''

comedy_movies = movies_in_genre('Comedy')

for genre in genres:
    comps = 0
    s = 0
    if genre != 'Comedy':
        movies_other = movies_in_genre(genre)
        for m1 in comedy_movies:
            for m2 in movies_other:
                s += compute_sim(m1,m2)
                comps += 1
        print(genre, s/comps)
