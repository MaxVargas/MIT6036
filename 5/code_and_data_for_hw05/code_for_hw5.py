...
def d_mean_square_loss_th(x, y, th, th0):
    ...
    #Your code here
    d,n = np.shape(x)
    return cv(np.sum(d_square_loss_th(x,y,th,th0),axis=1)/n)
...
def d_mean_square_loss_th0(x, y, th, th0):
    ...
    #Your code here
    d,n = np.shape(x)
    return cv(np.sum(d_square_loss_th0(x,y,th,th0),axis=1)/n)
...
def sgd(X, y, J, dJ, w0, step_size_fn, max_iter):
    ...
    for i in range(max_iter):
        ...
        w = ws[i].copy()
        fs.append(J(np.array([X[:,choice]]).T,np.array([y[:,choice]]).T,w))
        ws.append(w - step_size_fn(i)*dJ(np.array([X[:,choice]]).T,np.array([y[:,choice]]).T,w))
    return (ws[max_iter-1],fs[:],ws[:-1])
...
def sgdTest():
    ...
    return sgd(X, y, J, dJ, cv([0., 0.]), lambda i: 0.1, 1000)
