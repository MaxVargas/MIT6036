...
def run_keras_2d(data_name, layers, epochs, display=True, split=0.5, verbose=True, trials=1):
...
a = archs(3)
L = a[4]
run_keras_2d('3class', L, 10, trials=1, display=False)
#print(L[0].get_weights())
'''
...
X,Y = generate_1d_images(2,5,0.3)
print(X,Y,np.shape(Y))
...
'''
layer1 = Conv1D(filters=1, kernel_size=2, strides=1,use_bias=False, activation='relu', batch_size=1, input_shape=(128,1), padding='same',kernel_regularizer=filter_reg)
layer2 = MaxPooling1D(pool_size=2, strides=2, padding="valid")
Layers = [layer1,layer2,Flatten(),Dense(units=64, activation='relu', use_bias=False)]

D = get_image_data_1d(1000,128,0.1)

M,E = train_neural_counter(Layers,D)
...
def run_keras_cnn_mnist(train, test, layers, epochs, split=0.1, verbose=True, trials=1):
    ...
    m = X_train.shape[1]
    X_train = X_train.reshape((X_train.shape[0], m, m, 1))/256.
    X_val = X_val.reshape((X_val.shape[0], m, m, 1))/256.
    # Categorize the labels
    num_classes = 10
    y_train = np_utils.to_categorical(y1, num_classes)
    y_val = np_utils.to_categorical(y2, num_classes)
...
run_keras_fc_mnist(train_20, validation_20, layers, 1, split=0.1, verbose=True, trials=1)
...
#run_keras_cnn_mnist(train_20,validation_20,layers, 1, split=0.1, verbose=True, trials=1)
